...
namespace MySocket
{
    public class Socket_wrapper
    {
        ...
        private static string remoteHost = "192.168.1.5";
        private static int remotePort = 32770;
        ...
        /// <returns></returns>
        public static string socket_send(string sendMessage)
        {
            if (checkSocketState())
            {
        ...
        /// <returns></returns>
        public static string SendData(string dataStr)
        {
            string rev = "";
            lock (Locker1)
            {
                try
                {
                    if (!theSocket.Connected)
                    {
                        ...
                        //st.Disconnect(true);   
                        //st.Close();
                        //st = PLCSocket.SocketHelper.tcpConn("192.168.1.5", 32770, PLCSocket.PLCInfo.OutTime);
                    }
                    byte[] bytes = Encoding.ASCII.GetBytes(dataStr + "\r");
                    byte[] numArray = new byte[1000];

                    theSocket.Send(bytes);
                    theSocket.Receive(numArray);
